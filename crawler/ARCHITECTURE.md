...
```
                     ...
                     │  asyncio 事件循环 (单进程)                    │
   source_pages ──▶  │                                            │
   (任务注册表)       │  ucl.ac.uk 队列 ──▶ [3s节流] 1 worker       │      结果队列
                     │  gla.ac.uk 队列 ──▶ [3s节流] 1 worker       │ ──▶ ──────────▶ 消费线程
                     │  catalogue.leeds 队列 ▶ [1s节流] 3 workers  │      (串行解析/入库)
                     │  ... (最多 MAX_DOMAINS=10 个域并发)          │
                     └────────────────────────────────────────────┘
```

- **域内**：每个域名一条 FIFO 队列，按该域配置起 `concurrency` 个 worker（默认 1 = 串行）；
  共享一个域级节流器，保证两次请求**发起**间隔 ≥ `interval + 随机抖动(0~1s)`（默认 3s，可按域覆盖）。
  触发反爬时整个域一起指数退避（30s/60s/120s），不影响其他域。
  并发 >1 本质是牺牲礼貌换速度，只对实测无限流的域打开（YAML `domains` 按域配置）。
- **域间**：完全并发。10 所学校同时爬，总吞吐 ≈ 10 × (1页/3s) ≈ 12,000 页/小时，够用。
- **解析/入库在独立消费线程**：抓取协程只投递结果，解析和 MySQL 写入不阻塞事件循环；
  消费线程只有一个，数据库连接不跨线程。
