"""抓取编排：域名级并行 + 域内限速并发 + 反爬退避 + 响应分类 + 消费线程。

对外只有 fetch_tasks(tasks, handle)：tasks 按域名分队列，每域按 YAML 配置起
concurrency 个 worker 共享队列与域级节流器（两次请求发起间隔 >= interval + 抖动，
退避时全域暂停），域间并发（上限 MAX_DOMAINS）。handle(result) 在独立消费线程里
被逐个串行调用，解析/入库不阻塞事件循环。

"如何发一个请求"（方法分派/预热/拿票）在 methods.py；本文件只管编排与分类。
"""
import asyncio
import queue
import random
...
from fetcher import methods
from fetcher._util import domain as _domain
from fetcher._util import same_page as _same_page
...
async def _fetch_one(session, task):
    url = task.url
    try:
        async with await methods.send(session, task) as resp:
...
async def _fetch_with_retry(domain, task, session, throttle, log):
    ...
    # Cloudflare / 429 / WAF：全域暂停指数退避，用尽则失败留痕（只影响本域）
...
async def _run(tasks, emit, log):
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            await methods.browser.close()   # 若开过交互式浏览器会话，收尾关闭
