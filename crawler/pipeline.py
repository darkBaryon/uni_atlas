...
def _parse_and_load(conn, ldr, task, body, snapshot_id, report):
    ...
    for p in result.programs:
        ldr.load_program(p, task.id, snapshot_id, faculty_note=task.note)
...
def _handle_fetched(conn, ldr, task, res, report):
    ...
    if res.kind == "dead":
        ...
        _, created = registry.add_page(
            ...
            title=task.title, crawl_freq=task.crawl_freq,
            note=task.note)
        ...
        logger.warning("[失败·下轮自动重试] %s《%s》%s  原因: %s",
                       ...
                       (task.title or short)[:50], short, res.note)
    else:  # ok
        ...
        if not changed:
            registry.mark_fetched(conn, task.id, content_hash,
                                  changed=False, note=_clean_note(task.note))
        ...
        registry.mark_fetched(conn, task.id, content_hash,
                              changed=True, note=_clean_note(task.note))
...
def _school_list(tasks):
    ...
    for t in tasks:
        ...
        if code not in counts:
            order.append(code)
        counts[code] += 1
        labels[code] = t.uni_name_zh or code
